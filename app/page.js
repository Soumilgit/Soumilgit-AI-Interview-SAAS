...
export default function Page() {
  ...
  return <div className="min-h-screen overflow-x-hidden bg-background text-foreground">
    ...
    </div>{menuOpen && <nav className="border-t border-foreground/10 bg-background px-5 py-5 lg:hidden"><div className="flex flex-col gap-5"><div className="flex items-center justify-center gap-3 border-b border-foreground/10 pb-5"><iframe src="https://github.com/sponsors/Soumilgit/button" title="Sponsor SimulateRecruitAI" height="32" width="114" className="rounded border-0" /><a href={repo} target="_blank" rel="noreferrer" aria-label="GitHub repository" className="rounded-lg border border-foreground/20 p-2 text-foreground hover:bg-foreground hover:text-background"><FaGithub className="h-4 w-4" /></a></div>{nav("About", "about")}{nav("Features", "features")}{nav("Workflow", "workflow")}{nav("Insights", "insights")}{nav("FAQs", "faqs")}{nav("Contact", "contact")}</div></nav>}</header>
    <main>
      ...
      <section id="contact" className="scroll-mt-20 px-5 py-20 lg:px-8"><Contact /></section>
    </main>
