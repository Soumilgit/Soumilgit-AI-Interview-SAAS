...
export async function POST(request, { params }) {
  ...
  if (body.action === "transcribe") {
    if (typeof body.audio !== "string" || body.audio.length > 7_000_000 || body.mimeType !== "audio/webm") return NextResponse.json({ error: "Invalid audio." }, { status: 400 })
    try { return NextResponse.json({ transcription: await transcribeAudio(body.audio, body.mimeType) }) } catch { return NextResponse.json({ error: "Unable to transcribe audio." }, { status: 502 }) }
  }
